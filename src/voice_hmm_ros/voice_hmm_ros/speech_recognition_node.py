...
Módulo:      speech_recognition_node.py
...
Uso en ROS 2:
    ...
    3. Ejecutar el nodo pasándole los modelos entrenados:
       $ ros2 run voice_hmm_ros speech_recognition_node --model-dir src/voice_hmm_ros/resultados_hmm/models --codebook-path src/voice_hmm_ros/resultados_hmm/codebook.npy
...
class SpeechRecognitionNode(Node):
    def __init__(
        ...
        self.voice_valid_pub = self.create_publisher(
            Bool,
            "voice_command_valid",
            10
        )
    ...
    def capture_recognize_and_publish(self) -> None:
        """
        ...
        msg_word = String()
        msg_word.data = word
        self.voice_command_pub.publish(msg_word)

        # Mensaje de validez
        msg_valid = Bool()
        msg_valid.data = valid
        self.voice_valid_pub.publish(msg_valid)
...
def main(args=None) -> None:
    ...
    try:
        node.capture_recognize_and_publish()
    finally:
        node.destroy_node()
        rclpy.shutdown()
