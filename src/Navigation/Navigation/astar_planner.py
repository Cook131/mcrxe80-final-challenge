...
Nodo ROS 2 que combina:
  ...
  SUB:  /slam_map o /map   (nav_msgs/OccupancyGrid)
...
class AStarGrid:
    ...
    def heuristic(self, r0, c0, r1, c1) -> float:
        dr = abs(r1 - r0)
        dc = abs(c1 - c0)
    ...
    def _reconstruct(self, came_from, r, c):
        path = []
        node = (r, c)
...
class AStarPlannerNode(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(Pose2D,   goal_in_topic,   self._cb_goal,           10)
        self.create_subscription(Pose2D,   '/replan_trigger', self._cb_replan_trigger, 10)
        ...
        if target_topic != self.active_topic:
            with self.map_lock:
                if target_topic is None:
                    self.get_logger().warn('[A*] Se han perdido todas las fuentes de mapas.')
                    if self.sub_map is not None:
                        self.destroy_subscription(self.sub_map)
                        self.sub_map = None
                else:
                    self.get_logger().info(f'[A*] Conmutando mapa activo a: «{target_topic}»')
                    if self.sub_map is not None:
                        self.destroy_subscription(self.sub_map)
    ...
    def _cb_map(self, msg: OccupancyGrid):
        with self.map_lock:
            if self.map_msg is None:
                self.get_logger().info(
                    f'[A*] Mapa cargado — resolución: {msg.info.resolution:.3f}m | '
    ...
    def _cb_goal(self, msg: Pose2D):
        self.get_logger().info(f'[A*] Objetivo recibido: x={msg.x:.2f} y={msg.y:.2f}')
    ...
    def _plan_and_start(self, gx: float, gy: float):
        with self.map_lock:
            if self.map_msg is None:
                self.get_logger().warn('[A*] Sin mapa en memoria.')
                return
        ...
        sr = max(0, min(height - 1, sr));  sc = max(0, min(width - 1, sc))
        gr = max(0, min(height - 1, gr));  gc = max(0, min(width - 1, gc))

        cell_path = AStarGrid(grid, allow_diagonal=self.diagonal).plan((sr, sc), (gr, gc))
    ...
    def _inflate(grid: np.ndarray, radius: int) -> np.ndarray:
        ...
        inflated = binary_dilation(grid == 1, structure=struct)
        result  = grid.copy()
        result[inflated] = 1
        return result
